...
"""
def add_entry_to_config(cfgname,namespace,svcname, serviceId):
    #try:
        api_client = kubernetes.client.ApiClient()
        api_instance = kubernetes.client.CustomObjectsApi(api_client)
        ...
        api_response = api_instance.get_namespaced_custom_object("eks.amazonaws.com", "v1", namespace,
                                                                    "refactorspaceconfigs", cfgname)
        old_bindings = api_response["metadata"]["annotations"]["bindings"]
        entryList = old_bindings.split(",")
        entryList.append(serviceId+'.'+namespace +"."+svcname)
        new_bindings = ",".join(entryList)
        ...
        api_response = api_instance.patch_namespaced_custom_object("eks.amazonaws.com", "v1", namespace,
                                                                    "refactorspaceconfigs", cfgname, 
                                                                     service_patch)
        return api_response
    #except Exception as e:
...
"""
def delete_config_binding(cfgname,namespace,svcname, serviceId):
    #try:
        api_client = kubernetes.client.ApiClient()
        api_instance = kubernetes.client.CustomObjectsApi(api_client)
        ...
        resultList = set(entryList)-set(matchList)
        new_bindings =  (",".join(resultList))
